...
Use this only when direct observation shows the label changing without a click. Preview first; installation requires separate acknowledgement because it relies on an undocumented Desktop feature gate and adds a process-scoped session preload. The task running during installation is never restarted. The startup watcher is opt-in and observational; pass `-EnableStartupWatcher` only if you want the routing status watcher registered for login:
