...
Plugin-owned top-level settings live inside the generated `codex-safe-setup top-level` block, placed strictly before the first `[table]`; named profiles live in the `codex-safe-setup profiles` block at the end of the file. If the plan reports `SCOPE WARNING` findings, a key that belongs to Codex's top level (for example `model`, `notify`, or `service_tier`) sits under a wrong `[table]` — usually because an unrelated table was inserted above it. Explain the finding; offer `-RepairForeignMisplacedKeys` only after confirmation, and relay any `SCOPE CONFLICT` lines verbatim: those keys already exist at the top level, so only the user may decide which value wins. The installer refuses to write any candidate that fails its structural validation, and every write is protected by a plan-time SHA-256 check (`CONCURRENT_MODIFICATION` means something else edited config.toml mid-flight; rerun the plan).
