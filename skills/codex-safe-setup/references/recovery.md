...
A retired Desktop selector installation from version 0.2.0 or earlier keeps separate state at `CODEX_HOME/safe-setup/desktop-selector-fix.json` and `desktop-selector-fix-history`. `Remove-LegacyDesktopSelectorArtifacts.ps1 -PlanOnly` enumerates that residue; `-ConfirmApply` archives exact-match shortcuts, removes listed user-scope `CSS_DESKTOP_SELECTOR_*` environment variables, and moves the retired state into `legacy-selector-quarantine/`. The cleanup never touches processes or any file of the official client.
