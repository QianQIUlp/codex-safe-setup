...
$dynamicUiCatalogSupported = $false
if ($codexCli.Version -and $codexCli.Version -match '(\d+\.\d+\.\d+)') {
    $detectedCodexVersion = [version]$Matches[1]
    $codexVersionSupported = $detectedCodexVersion -ge [version]'0.138.0'
    $dynamicUiCatalogSupported = $detectedCodexVersion -ge [version]'0.138.0'
}
...
$managedFilesystemSectionText = Get-CssTomlSectionText -Text $configText -Section 'permissions.codex-safe-workspace.filesystem'
...
$dynamicUiDenyMergeRisk = -not $legacy.Present -and $catalogProfileAvailable -and ($managedProfileHasExplicitDeny -or $offlineProfileHasExplicitDeny) -and $profileSettings.DefaultProfile -in @(':read-only', ':workspace', ':danger-full-access')
$fullAccessRequested = ($profileSettings.DefaultPresent -and $profileSettings.DefaultProfile -eq ':danger-full-access') -or ($sandboxModeSetting.Present -and $sandboxModeSetting.Value -eq 'danger-full-access')
$fullAccess = $fullAccessRequested -and -not $permissionConfigurationConflict -and -not $dynamicUiDenyMergeRisk
...
$managedProfileActive = $managedProfileAvailable -and $profileSettings.DefaultPresent -and $profileSettings.DefaultProfile -eq $script:CssProfileName -and -not $permissionConfigurationConflict
$legacyNetworkSectionText = Get-CssTomlSectionText -Text $configText -Section 'sandbox_workspace_write'
$dynamicUiStructurePresent = -not $legacy.Present -and $profileSettings.DefaultProfile -eq ':workspace' -and $profileSettings.ManagedProfilePresent -and $profileSettings.OfflineProfilePresent -and -not $managedProfileHasExplicitDeny -and -not $offlineProfileHasExplicitDeny
$dynamicUiRoutingReady = $dynamicUiStructurePresent -and $dynamicUiCatalogSupported
$permissionRouting = if ($permissionConfigurationConflict -or $dynamicUiDenyMergeRisk) { 'Conflict' } elseif ($dynamicUiStructurePresent) { 'DynamicUi' } elseif ($managedProfileActive) { 'StrictProfile' } else { 'Unknown' }
...
$networkEnabled = if ($permissionRouting -eq 'Unknown' -and $legacy.Present) { $legacyNetworkSectionText -match '(?m)^\s*network_access\s*=\s*true' } else { $profileNetworkSectionText -match '(?m)^\s*enabled\s*=\s*true' }
...
if ($fullAccess) { $findings.Add([pscustomobject]@{ Severity = 'HIGH'; Finding = 'The configured fallback/default is Full Access, which removes the local sandbox boundary when no task override replaces it.' }) }
if ($permissionConfigurationConflict) { $findings.Add([pscustomobject]@{ Severity = 'HIGH'; Finding = 'Legacy sandbox keys and default_permissions coexist. The visible Full Access selection is not authoritative because the named profile can remain pinned or be conflict-merged.' }) }
if ($dynamicUiDenyMergeRisk) { $findings.Add([pscustomobject]@{ Severity = 'HIGH'; Finding = 'The DynamicUi Custom profile contains an explicit filesystem deny. Codex preserves that deny during profile changes, so a later Full Access selection can remain managed and restricted.' }) }
...
if ($permissionRouting -eq 'DynamicUi') {
    $findings.Add([pscustomobject]@{ Severity = 'MEDIUM'; Finding = 'DynamicUi uses two positive-only named profiles and built-in Workspace as the startup default. Runtime selection remains separate from visible Desktop label stability; workspace credential files remain readable while the workspace is writable.' })
}
elseif (-not $managedProfileAvailable) {
...
elseif ($permissionRouting -eq 'DynamicUi' -and -not $dynamicUiCatalogSupported) { $findings.Add([pscustomobject]@{ Severity = 'HIGH'; Finding = 'DynamicUi requires Codex CLI 0.138.0 or newer.' }) }
...
$report = [pscustomobject]@{
    ...
    FullAccessRequested = [bool]$fullAccessRequested
    PermissionConfigurationConflict = [bool]$permissionConfigurationConflict
    PermissionProfile = $profileSettings.DefaultProfile
    SelectableCustomProfile = $(if ($dynamicUiStructurePresent) { $script:CssProfileName } elseif ($catalogProfileAvailable) { $script:CssProfileName } else { $null })
    PermissionRouting = $permissionRouting
    DynamicUiRoutingReady = [bool]$dynamicUiRoutingReady
    DynamicUiDenyMergeRisk = [bool]$dynamicUiDenyMergeRisk
    RuntimePermissionSelection = 'NOT OBSERVED: this config assessment cannot see a task-level UI override. A settings event may corroborate the selector, but only the real turn_context, command result, completion event, and outside-workspace canary establish effective permission.'
    ManagedLeastPrivilegeProfile = [bool]$managedProfileActive
    ManagedProfileAvailable = [bool]$managedProfileAvailable
    CustomCatalogProfileAvailable = [bool]($dynamicUiStructurePresent -or $catalogProfileAvailable)
    ...
    DynamicUiCatalogSupported = [bool]$dynamicUiCatalogSupported
    Findings = $findings
}
...
Write-Output ("Configured fallback/default Full Access: {0}" -f $report.ConfiguredDefaultFullAccess)
Write-Output ("Permission routing: {0}; default profile: {1}; selectable Custom: {2}" -f $report.PermissionRouting, $(if ($report.PermissionProfile) { $report.PermissionProfile } else { '<not set>' }), $(if ($report.SelectableCustomProfile) { $report.SelectableCustomProfile } else { '<not available>' }))
