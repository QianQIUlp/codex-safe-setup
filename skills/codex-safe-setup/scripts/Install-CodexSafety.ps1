...
param(
    ...
    [switch]$RepairForeignMisplacedKeys,
    [switch]$PlanOnly,
    [switch]$ConfirmApply,
...
$originalConfigSha256 = if (Test-Path -LiteralPath $resolvedConfig -PathType Leaf) {
    Get-CssFileTextSha256 -Text $existingConfig
} else { '' }
...
$newConfig = Repair-CssMisplacedPluginKeys -Text (Remove-CssAllManagedRegions -Text $existingConfig)
$newConfigText = $newConfig.Text
$newConfig = Remove-CssTomlSections -Text $newConfigText -SectionPrefixes @("permissions.$($script:CssProfileName)", "permissions.$($script:CssOfflineProfileName)", 'features.network_proxy')
...
# Pre-flight scope diagnostics: plugin-owned misplacements are repaired above;
# foreign ones are surfaced in the plan and block nothing by themselves.
$originalDiagnosticDocument = Get-CssTomlDocument -Text $existingConfig -IncludeDiagnostics
$originalDiagnostics = @($originalDiagnosticDocument.Diagnostics | Where-Object { $_.Code -eq 'MisplacedTopLevelKey' })
$foreignMisplacements = @($originalDiagnostics | Where-Object Severity -eq 'Warning')
$pluginMisplacements = @($originalDiagnostics | Where-Object Severity -eq 'Error')

if ($RepairForeignMisplacedKeys -and $foreignMisplacements.Count -gt 0) {
    $originalDocument = Get-CssTomlDocument -Text $existingConfig
    $foreignLineNumbers = @($foreignMisplacements | ForEach-Object { $_.LineNumber })
    $foreignSourceLines = @($originalDocument.Elements | Where-Object {
        $_.Kind -eq 'KeyValue' -and $foreignLineNumbers -contains ($_.Index + 1)
    } | ForEach-Object Text)
}
...
# Managed top-level block: strictly before the first [table], owning the keys
# whose scope was the root cause of the 2026-08 swallowed-key incident.
# Stray same-name keys left by earlier writers must go first so the managed
# block is the single source of truth for these settings.
$newConfig = Remove-CssTomlTopLevelKeys -Text $newConfig -Keys ([string[]]$script:CssPluginOwnedTopLevelKeys)
$topBlockLines = [Collections.Generic.List[string]]::new()
$topBlockLines.Add($script:CssTomlTopBlockStart)
$topBlockLines.Add('# Generated by Install-CodexSafety.ps1. Re-run the installer instead of editing this block.')
if ($PermissionRouting -eq 'DynamicUi') {
    $topBlockLines.Add('# DynamicUi startup default stays on built-in Workspace; two named profiles follow below.')
    $topBlockLines.Add('default_permissions = ":workspace"')
}
else {
    $topBlockLines.Add('default_permissions = ' + (ConvertTo-CssTomlString $script:CssProfileName))
    $topBlockLines.Add('approval_policy = ' + (ConvertTo-CssTomlString $approvalPolicy))
    $topBlockLines.Add('approvals_reviewer = ' + (ConvertTo-CssTomlString $approvalReviewer))
}
$topBlockLines.Add($script:CssTomlTopBlockEnd)
$newConfig = Set-CssTomlTopLevelBlock -Text $newConfig -BlockLines $topBlockLines.ToArray()

if ($PermissionRouting -eq 'DynamicUi') {
    $managedLines = [Collections.Generic.List[string]]::new()
    $managedLines.Add($script:CssTomlProfileStart)
...
$managedLines.Add($script:CssTomlProfileEnd)
$newConfig = Set-CssTomlProfilesBlock -Text $newConfig -BlockLines $managedLines.ToArray()

$relocationSkipped = @()
if ($RepairForeignMisplacedKeys -and $foreignMisplacements.Count -gt 0) {
    $relocationResult = Add-CssTomlRelocatedUserLines -Text $newConfig -SourceLines $foreignSourceLines
    $newConfig = $relocationResult.Text
    $relocationSkipped = @($relocationResult.SkippedKeys)
}

# Candidate validation: the assembled configuration must be structurally clean
# before it is ever shown in a plan or written to disk.
$candidateDocument = Get-CssTomlDocument -Text $newConfig -IncludeDiagnostics
$candidateDiagnostics = @($candidateDocument.Diagnostics)
$fatalCandidateDiagnostics = @($candidateDiagnostics | Where-Object Severity -eq 'Error')
if ($fatalCandidateDiagnostics.Count -gt 0) {
    $detail = ($fatalCandidateDiagnostics | ForEach-Object { "[{0}] line {1}: {2}" -f $_.Code, $_.LineNumber, $_.Message }) -join [Environment]::NewLine
    throw ("Generated candidate configuration failed structural validation and nothing was changed:{0}{1}" -f [Environment]::NewLine, $detail)
}
...
if ($pluginMisplacements.Count -gt 0) {
    Write-Output ('Scope repair: {0} plugin-owned key(s) found under wrong [tables] will be relocated into the managed top-level block:' -f $pluginMisplacements.Count)
    foreach ($finding in $pluginMisplacements) {
        Write-Output ("  - '{0}' under [{1}] (line {2})" -f $finding.Key, $finding.SectionPath, $finding.LineNumber)
    }
}
foreach ($finding in $foreignMisplacements) {
    Write-Output ("SCOPE WARNING: foreign key '{0}' sits under [{1}] (line {2}) and is ignored by this installer." -f $finding.Key, $finding.SectionPath, $finding.LineNumber)
    Write-Output '  Fix it manually, or rerun with -RepairForeignMisplacedKeys to relocate the exact line to the TOML top level.'
}
foreach ($skippedKey in $relocationSkipped) {
    Write-Output ("SCOPE CONFLICT: '{0}' was not relocated because the same key already exists at the TOML top level. Resolve the duplicate by hand." -f $skippedKey)
}
if ($PlanOnly) {
...
$canaryTouched = $true
try {
    Invoke-CssGuardedConfigWrite -Path $resolvedConfig -ExpectedOriginalSha256 $originalConfigSha256 -NewText $newConfig
