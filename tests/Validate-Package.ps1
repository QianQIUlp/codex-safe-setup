...
Assert-True ($desktopSelectorLauncherText -match 'Get-SequenceCount' -and $desktopSelectorLauncherText.IndexOf('ValidateOnly', [StringComparison]::Ordinal) -lt $desktopSelectorLauncherText.IndexOf('prospectiveRoute', [StringComparison]::Ordinal) -and $desktopSelectorWatcherText -match 'MANUAL_ACTION_REQUIRED' -and $desktopSelectorWatcherText -match 'Get-RoutingObservation' -and $desktopSelectorWatcherText -notmatch 'CloseMainWindow|Stop-Process|Start-CodexFixed' -and $desktopSelectorInstallText -match 'watcherReady') 'The hidden validation must traverse live process routing, scalar query results must be normalized, the watcher must stay observational (manual-action status, never closing or restarting Desktop), and installation must prove watcher liveness when opted in.'
Assert-True ($desktopSelectorLauncherText -notmatch 'CloseMainWindow' -and $desktopSelectorLauncherText -notmatch 'Stop-Process' -and $desktopSelectorLauncherText -match 'MANUAL_ACTION_REQUIRED') 'The launcher must fail closed on uninstrumented official Desktop processes and never close or force-terminate them.'
Assert-True ($desktopSelectorInstallText.IndexOf('Remove-CssDesktopSelectorLegacyShortcuts', [StringComparison]::Ordinal) -ge 0 -and $desktopSelectorInstallText.IndexOf('Remove-CssDesktopSelectorLegacyShortcuts', [StringComparison]::Ordinal) -lt $desktopSelectorInstallText.IndexOf('Stop-CssDesktopSelectorWatcher', [StringComparison]::Ordinal) -and $desktopSelectorInstallText -match 'EnableStartupWatcher') 'Installation must archive and remove provable legacy autostart entries before any state change and make the startup watcher opt-in.'
Assert-True ($desktopSelectorRollbackText -notmatch 'Start-Process') 'Rollback must never launch a watcher or resurrect autostart entries.'
