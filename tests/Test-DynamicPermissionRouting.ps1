...
try {
    ...
    Assert-Routing ($dynamicInstallState.schemaVersion -eq 9 -and $dynamicInstallState.productVersion -eq '0.2.0') 'DynamicUi install state must be version 0.2.0 schema 9.'
...
'@, [Text.UTF8Encoding]::new($false))
    ...
    Assert-Routing ($repairedState.schemaVersion -eq 9 -and $repairedState.productVersion -eq '0.2.0' -and $repairedState.PermissionRouting -eq 'DynamicUi' -and $repairedState.DynamicUiDisplayRoute -eq 'DualNamedProfiles') 'Upgrade must record the 0.2.0 DynamicUi state.'
    ...
    Assert-Routing ($releasedState.schemaVersion -eq 9 -and $releasedState.productVersion -eq '0.2.0') 'The unreleased 0.1.9 schema-8 state must migrate to the 0.2.0 release state.'
    ...
    Write-Output 'PASS: unreleased schema-8 DynamicUi state migrates to 0.2.0 schema 9'
