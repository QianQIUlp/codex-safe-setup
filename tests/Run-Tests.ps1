...
"@
    ...
    $legacyConfig = $legacyConfig -replace '(?m)^network_proxy = false -Repository $repository -GitArguments @('init') | Out-Null
...
, 'network_proxy = true'
    $legacyConfig = $legacyConfig -replace '(?m)^enabled = false\r?\n# <<< codex-safe-setup managed <<< -Repository $repository -GitArguments @('init') | Out-Null
    Invoke-GitTest -Repository $repository -GitArguments @('config', 'user.name', 'Test User') | Out-Null
    Invoke-GitTest -Repository $repository -GitArguments @('config', 'user.email', 'test@example.invalid') | Out-Null
    [IO.File]::WriteAllText((Join-Path $repository 'tracked.txt'), 'original', [Text.UTF8Encoding]::new($false))
    Invoke-GitTest -Repository $repository -GitArguments @('add', 'tracked.txt') | Out-Null
...
, ('enabled = true' + [Environment]::NewLine + [Environment]::NewLine + '[permissions.codex-safe-workspace.network.domains]' + [Environment]::NewLine + '"*" = "allow"' + [Environment]::NewLine + '# <<< codex-safe-setup managed <<<')
    ...
    Assert-True ($upgradedConfig -match '(?m)^network_proxy = false -Repository $repository -GitArguments @('init') | Out-Null
    Invoke-GitTest -Repository $repository -GitArguments @('config', 'user.name', 'Test User') | Out-Null
    Invoke-GitTest -Repository $repository -GitArguments @('config', 'user.email', 'test@example.invalid') | Out-Null
    [IO.File]::WriteAllText((Join-Path $repository 'tracked.txt'), 'original', [Text.UTF8Encoding]::new($false))
    Invoke-GitTest -Repository $repository -GitArguments @('add', 'tracked.txt') | Out-Null
